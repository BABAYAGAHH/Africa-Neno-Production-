    ...
    <div className="glass-panel p-6 sm:p-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h2 className="mt-3 font-display text-3xl text-white sm:text-4xl">Reserve your date</h2>
        </div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          ...
          <label className="space-y-2 text-sm text-white/78">
            ...
            <select name="serviceType" value={formData.serviceType} onChange={handleChange} className="field-input">
              {serviceOptions.map((option) => (
                <option key={option} value={option} className="bg-brand-darker text-white">
                  {option}
                </option>
              ))}
            </select>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm leading-7 text-white/58">We usually respond to booking requests within 24 hours.</p>
